...
struct threadArgs {
	...
	unsigned int squaredId; // the new variable
};
...
void* child(void* params) {
	...
	unsigned int squaredId = childID * childID; //the child id squared
	args->squaredId = squaredId;
	return (void*)args; //return a pointer to the struct
}
...
int main(int argc, char** argv) {
	...
	for (unsigned int id = 0; id < numThreads; id++) {
		printf("The squared id is %u \n", args[id].squaredId); // print the squared id for the threads
	}
