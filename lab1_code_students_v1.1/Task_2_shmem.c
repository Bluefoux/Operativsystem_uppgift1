...
int main(int argc, char **argv)
{
	...
	**/
	...
	char *addr = NULL;
	pid_t pid = -1;
	int shmid = -1, buffernmrtwo = 0, randomnmr, counter=0;
	...
	if (pid != 0) {
		...
		while (shmp->var1 < 100) 
		{
			if (shmp->var1 < (shmp->var2 + 9)){ // if the buffer contains less than 9 numbers
				/* write to shmem */
				shmp->var1++;
				printf("Sending %d\n", shmp->var1); fflush(stdout);
				shmp->buffer[shmp->buffernmr] = shmp->var1;
				shmp->buffernmr++;
