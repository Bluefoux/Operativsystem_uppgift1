...
int main(int argc, char **argv)
{
	...
	**/
	struct shm_struct {
		int buffer[10]; // <-- fel (den ska vara cirkulär och inte skriva över tidigare skrivna tal(elr något liknande) )
		int buffernmr;
	};
	volatile struct shm_struct *shmp = NULL;
	char *addr = NULL;
	pid_t pid = -1;
	int var1 = 0, var2 = 0, shmid = -1, status, randomnmr, buffernmrtwo=0;
